...
#define MAVLINK_MSG_ID_TARGET_OFFBOARD 183
...
static inline uint16_t mavlink_msg_target_offboard_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint32_t time_boot_ms, float pos_x, float pos_y, float pos_z, float head, uint8_t target_system, uint8_t target_component, uint8_t led_type, uint8_t led_speed, uint8_t led_r, uint8_t led_g, uint8_t led_b, uint8_t led_bright)
{
...
{
    return mavlink_msg_target_offboard_pack(system_id, component_id, msg, target_offboard->time_boot_ms, target_offboard->pos_x, target_offboard->pos_y, target_offboard->pos_z, target_offboard->head, target_offboard->target_system, target_offboard->target_component, target_offboard->led_type, target_offboard->led_speed, target_offboard->led_r, target_offboard->led_g, target_offboard->led_b, target_offboard->led_bright);
}
...
static inline void mavlink_msg_target_offboard_send(mavlink_channel_t chan, uint32_t time_boot_ms, float pos_x, float pos_y, float pos_z, float head, uint8_t target_system, uint8_t target_component, uint8_t led_type, uint8_t led_speed, uint8_t led_r, uint8_t led_g, uint8_t led_b, uint8_t led_bright)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_TARGET_OFFBOARD_LEN];
    _mav_put_uint32_t(buf, 0, time_boot_ms);
 ...
 */
static inline void mavlink_msg_target_offboard_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t time_boot_ms, float pos_x, float pos_y, float pos_z, float head, uint8_t target_system, uint8_t target_component, uint8_t led_type, uint8_t led_speed, uint8_t led_r, uint8_t led_g, uint8_t led_b, uint8_t led_bright)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
